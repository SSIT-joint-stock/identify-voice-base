...
- **INSERT User**: Tạo bản ghi mới trong bảng `users` với các Metadata đang có trong `ai_identities_cache` (Name, SĐT...).
...
Endpoint hiện tại không nhận body. Backend chuyển đổi trực tiếp từ `voice_id` trong path và metadata đã có trong `ai_identities_cache`.

```http
POST /api/v1/ai-voices/old-ai-voice-id/convert
Authorization: Bearer <access_token>
```

Lưu ý: bảng `ai_identities_cache` hiện chưa lưu `age` và `gender`; khi convert, các trường này dùng default từ bảng `users` (`age = 0`, `gender = OTHER`) cho đến khi được cập nhật qua API `PATCH /api/v1/voices/:id`.
