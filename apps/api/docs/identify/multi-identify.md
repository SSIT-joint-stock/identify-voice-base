...
- Hệ thống sẽ trả về mảng `speakers` chứa thông tin của tất cả các Speaker được phát hiện.
...
```json
{
  ...
  "data": {
    ...
    "audio_url": "https://cdn.example.com/uploads/identify/2026/04/multi.wav",
    "identified_at": "2026-04-11T15:05:00Z",
    "type": "MULTI",
    "speakers": [
      {
      ...
      },
      {
        ...
        "matched_voice_id": "9f5cf696-...",
        "score": 0.8845,
        "name": "AI Identity #4021",
        "audio_url": "https://api.example.com/api/v1/sessions/session-uuid-multi/speakers/speaker_1/audio"
      }
    ]
  }
}
...
- FE sử dụng `speaker_label` (ví dụ `speaker_0`) từ kết quả trả về để gọi link audio này.

### 5.3 Trạng thái Identification

Nếu kết quả trả về có Speaker thiếu `matched_voice_id`, đó là "Khách lạ" hoàn toàn (Unknown). Nếu có `matched_voice_id` nhưng thiếu thông tin chi tiết (hoặc thiếu `enroll_audio_url`), đó là một danh tính đã được AI ghi nhớ nhưng chưa được Operator xác thực. FE nên hiển thị rõ các trạng thái này.
